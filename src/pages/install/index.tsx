...
import { getTestData, getTreeData } from '@/service/install';
...
export default function Hello() {
  ...
  const [treeDataDirection, setTreeDataDirection] = useState<any>([
    {
      id: 1,
      pId: 0,
      value: '1',
  ...
  const onLoadData: any = ({ id }) =>
    new Promise((resolve) => {
      setTimeout(() => {
        getTreeData({ parentId: id }).then((res) => {
          console.log(treeDataDirection.concat(res?.list));
          setTreeDataDirection(treeDataDirection.concat([...res?.list]));
        });
        // setTreeDataDirection(
        //   treeDataDirection.concat([
  ...
  const onChange = (newValue, n2) => {
    console.log(newValue, n2);
    setSelectedDirectory(newValue);
  };
  ...
  const onCheck = (checkedKeysValue: any) => {
    ...
    const sortedArray = checkedKeysValue
      ?.filter((i: any) =>
        ['企业管理器', '服务器', '数据库迁移服务', '数据库监控服务']?.includes(
          i,
        ),
      )
      .slice()
          ...
          <div className="centered-container-second">
            ...
            <div className="bottom-right">
              ...
              <Button
                ...
                onClick={() => {
                  nextStep();
                  console.log(formValues, jumpKeys);
                  jumpKeys?.map((i, index) => {
                    if (i !== null) {
                      getTestData({
                        ...formValues['step' + Number(index + 1)],
                        i,
                      }).then((res) => {
                        console.log(res, i);
                        finish?.map((ii: Record<string, string>) => {
                          if (ii?.name === i) {
                            ii.finish = '成功';
                          }
                        });
        ...
        pageName === 'setuping' && (
          <div className="centered-container-second">
            ...
            >
              ...
              {finish !== null &&
              finish?.length !== 0 &&
              finish?.filter((i: Record<string, string>) =>
              ...
              <Button
                ...
                disabled={
                  !(
                    finish !== null &&
                    finish?.length !== 0 &&
                    finish?.every((i: Record<string, string>) =>
                      ['成功', '失败']?.includes(i?.finish),
                    )
                  )
                }
